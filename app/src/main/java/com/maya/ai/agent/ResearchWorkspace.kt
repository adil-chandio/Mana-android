...
/** Reusable same-process research UI. Host owns visibility/lifecycle; no intent/session transfer. */
class ResearchWorkspace(private val host: AppCompatActivity) : ContextWrapper(host) {
    ...
    fun createView(): View {
        ...
        goal.addTextChangedListener(watcher);plan.addTextChangedListener(watcher);paint();return shell
    }
    ...
    val hasContent get() = goal.text.isNotEmpty() || plan.text.isNotEmpty() || runner.results().isNotEmpty() || summary.text.isNotEmpty()
    fun setGoal(value: String): Boolean {
        if(!foreground || value.isBlank() || value.length>400 || !com.maya.ai.chat.NativeChatProtocol.validReply(value)) return false
        goal.setText(value);return true // deliberate local copy only; no AI request/approval
    }
    fun resume() {foreground=true;paint()}
    fun pause() {foreground=false;stopWork()}
    fun dispose() {pause();clear();handler.removeCallbacksAndMessages(null)}
    fun focusChanged(hasFocus: Boolean) {if(!hasFocus && (runner.busy || cancelModel!=null)) stopWork()}
