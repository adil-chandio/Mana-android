...
class InlineBuildTurn(private val host: AppCompatActivity, initialGoal: String,
    ...
    override val view=LinearLayout(host).apply {
        ...
        setPadding(dp(12),dp(12),dp(12),dp(12));setBackgroundColor(Color.rgb(27,32,43))
    }
    ...
    init {
        label("AGENT · STATIC PAGE BUILDER",14f);label(initialGoal,16f)
        label("One local index.html in this conversation. HTML/CSS static preview only: no JavaScript, remote images, internet, files or phone actions. AI is a small 256-token prototype, not a complete website/IDE.",12f)
        status=label("No AI request or preview started.",13f).apply {tag="builder_status";accessibilityLiveRegion=View.ACCESSIBILITY_LIVE_REGION_POLITE}
        label("FILE · index.html (memory only, maximum 8,000 characters)",12f)
        editor=EditText(host).apply {
            tag="builder_code";isSaveEnabled=false;setTextColor(Color.WHITE);setHintTextColor(Color.LTGRAY)
            hint="Paste or edit HTML here. Preview requires confirmation.";minLines=3;maxLines=7
        ...
        button("Ask AI for code · consent") {propose(goal)}
        proposalView=label("",13f).apply {tag="builder_proposal";setTextIsSelectable(true)}
        ...
        previewBox=LinearLayout(host).apply {orientation=LinearLayout.VERTICAL;tag="builder_preview_area";isSaveEnabled=false;view.addView(this)}
        label("To revise: keep Agent / Build page selected, type the change in the SAME message box and submit. AI sees only the shown current code plus that request after consent. Larger code stays locally editable; it is never silently shortened to fit AI.",12f)
    ...
    private fun requestCode(request: String, code: String) {
        ...
        proposed="";proposalView.text="";val ticket=++epoch;started=SystemClock.elapsedRealtime()
    ...
    private fun renderPreview(code: String) {
        ...
        preview=browser;previewBox.addView(browser,LinearLayout.LayoutParams(-1,dp(280)))
    ...
    private fun confirm(title: String,message: String,action: () -> Unit) {
        ...
        dialog=AlertDialog.Builder(host).setTitle(title).setMessage(message).setNegativeButton("Cancel",null)
            .setPositiveButton("Confirm") {_,_->if(canAct() && ticket==epoch) {epoch++;action()}}.create().also {d ->
                d.setOnDismissListener {if(ticket==epoch) epoch++};d.show();d.getButton(AlertDialog.BUTTON_POSITIVE).filterTouchesWhenObscured=true
    ...
    override fun refresh() {
        val enabled=canAct();buttons.forEach {it.isEnabled=enabled};editor.isEnabled=enabled
        ...
        proposalView.visibility=if(proposed.isEmpty()) View.GONE else View.VISIBLE
        apply.visibility=proposalView.visibility
    }
    ...
    private fun label(value: String,size: Float)=TextView(host).apply {text=value;textSize=size;setTextColor(Color.WHITE);isSaveEnabled=false;setPadding(0,dp(5),0,dp(5));view.addView(this)}
