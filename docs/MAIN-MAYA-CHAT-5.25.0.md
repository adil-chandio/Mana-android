...
- Opening the workspace requires the existing fixed navigation URI from a main-frame, user-gesture request on the trusted packaged page. No URL query/payload, draft, history or credentials are forwarded. Real fixed-URI anchors preserve the native WebView user-gesture signal (no scripted location assignment); synthetic clicks cannot launch through the new handler, and the native gate independently checks gesture/frame/origin.
...
- **MAYA HOME** / Back uses the native exit confirmation whenever either workspace has content. Closing clears Chat/Agent, destroys that native pane, restores the original surface and performs only a fixed trusted `showTab('tab-home')` UI command. Navigation/hosting transfers no private text into JavaScript, DOM storage or legacy diagnostics. The pre-existing, explicitly consented Sunao preparation still passes only its selected text through the trusted local Fish/BOLI conversion and restores BOLI.last, unchanged.
