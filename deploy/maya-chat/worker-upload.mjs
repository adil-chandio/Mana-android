...
function modelText(output) {
  const reject = () => {
    throw new Failure(502, "INVALID_MODEL_RESPONSE");
  };
  if (!object(output) || output.object !== "chat.completion" || !Array.isArray(output.choices) || output.choices.length !== 1 || !noTools(output.tool_calls) || output.function_call !== void 0) reject();
  const choice = output.choices[0];
  if (!object(choice) || choice.index !== 0 || choice.finish_reason !== "stop" || !noTools(choice.tool_calls) || choice.function_call !== void 0) reject();
  const message = choice.message;
  if (!object(message) || message.role !== "assistant" || !noTools(message.tool_calls) || message.function_call !== void 0 || message.refusal !== void 0 && message.refusal !== null) reject();
  if (message.reasoning_content !== void 0 && message.reasoning_content !== null && (typeof message.reasoning_content !== "string" || message.reasoning_content.length > LIMITS.outputChars)) reject();
...
<p id="status" role="status" aria-live="polite">Ready for local setup. No AI request sent.</p><p class="fine">STOP ends this tab's wait. Once sent, remote work may continue and usage may count. A failed message is not added to future context. There is no automatic resend.</p></section>
<footer>Local candidate: Qwen3-30B-A3B-FP8 via Cloudflare Workers AI. Model weights: Apache-2.0. No live inference verified for this candidate.<br>Use the registered browser. Never paste private keys, passwords, OTPs or provider tokens here.</footer>
