...
<footer>Use the registered browser. Never paste private keys, passwords, OTPs or provider tokens here.</footer>
      ...
      return await bounded2(request, clock, async (scope) => {
        ...
        try {
          output = await env.AI.run(MODEL, {
            messages: [{ role: "system", content: SYSTEM }, ...auth.messages],
        ...
        scope.check();
        if (!output || typeof output.response !== "string" || !output.response.trim() || output.response.length > LIMITS.outputChars || output.tool_calls !== void 0 && (!Array.isArray(output.tool_calls) || output.tool_calls.length)) throw new Failure(502, "INVALID_MODEL_RESPONSE");
        return reply({
          requestId,
          kind: "model-response",
          model: MODEL,
